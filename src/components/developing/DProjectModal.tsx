...
export const DProjectModal = ({
  ...
  const content = (
    ...
    >
      ...
      >
        <img className="w-full" src={imgGif} alt={`An image of the ${title} project.`} />
        <div className="p-8">
          <h4 className="text-3xl font-bold mb-2">{title}</h4>
          <div className="flex flex-wrap gap-2 text-sm text-sky-300">
            {tech.join(" - ")}
          </div>
          ...
          <div>
            <p className="font-bold mb-2 text-xl">
              Project Links<span className="text-sky-500">.</span>
            </p>
            <div className="flex items-center gap-4 text-sm">
              {code && (
                <Link
                  target="_blank"
                  rel="nofollow"
                  className="text-zinc-300 hover:text-sky-300 transition-colors flex items-center gap-1"
                  href={code}
                >
